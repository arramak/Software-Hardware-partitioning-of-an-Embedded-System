...
export  PGA_2_Start
export _PGA_2_Start
export  PGA_2_SetPower
export _PGA_2_SetPower

export  PGA_2_SetGain
export _PGA_2_SetGain

export  PGA_2_Stop
export _PGA_2_Stop

IF (PGA_2_AGNDBUFAPI)
export  PGA_2_EnableAGNDBuffer
export _PGA_2_EnableAGNDBuffer
export  PGA_2_DisableAGNDBuffer
export _PGA_2_DisableAGNDBuffer
ENDIF
...
;
   ...
   mov  A, reg[PGA_2_GAIN_CR2]                             ; read power value
   and  A, ~POWERMASK                                ; clear power bits in A
   or   A, [X]                                       ; combine power value with balance of reg.
   mov  reg[PGA_2_GAIN_CR2], A                        ; move complete value back to register
   pop  A
...
;
   ...
   mov  A, reg[PGA_2_GAIN_CR0]                             ; read mux settings
   and  A, ~GAINREGMASK                                   ; clear gain bits in A
   tst  [X],HIGHGAIN                                      ; See if High Gain is set
   jnz  .SETHIGHGAIN
   and  reg[PGA_2_GAIN_CR3],~EXGAIN                        ; Clear High Gain bit.
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[PGA_2_GAIN_CR0], A                             ; move complete value back to register
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
...
.SETHIGHGAIN:
   ...
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[PGA_2_GAIN_CR0], A                             ; move complete value back to register
   or   reg[PGA_2_GAIN_CR3], EXGAIN                        ; Set High Gain bit.
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
.ENDSECTION
...
_PGA_2_Stop:
   ...
   and REG[PGA_2_GAIN_CR2], ~POWERMASK
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: PGA_2_EnableAGNDBuffer
;
;  DESCRIPTION:
...
;    functions.
;
 PGA_2_EnableAGNDBuffer:
_PGA_2_EnableAGNDBuffer:
   RAM_PROLOGUE RAM_USE_CLASS_1
   and   reg[PGA_2_GAIN_CR3], ~AGNDBUFMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: PGA_2_DisableAGNDBuffer
;
;  DESCRIPTION:
...
;    functions.
;
 PGA_2_DisableAGNDBuffer:
_PGA_2_DisableAGNDBuffer:
   RAM_PROLOGUE RAM_USE_CLASS_1
   or    reg[PGA_2_GAIN_CR3], AGNDBUFMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
ENDIF

; End of File PGA_2.asm
